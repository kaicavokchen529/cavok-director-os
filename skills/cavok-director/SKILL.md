...
10. Build a scene-level Camera Language Brief before individual coverage: viewpoint baseline and arc, camera temperament, expression permission, visual-distance curve, support and optical palette, expressive-technique opportunities, platform handoffs, and repeated defaults to avoid. Then choose framing, position, roll, support, optics, movement topology, duration, landing, and transition for each shot. For hyperreal combat, every conspicuous camera move must have a performer, force, spatial, or information trigger.
...
- Scene-level cinematography strategy, first-person/semi-subjective/objective/aerial regimes, shot-size arc, Dutch angle, zoom family, Steadicam/handheld/body rigs, drone/FPV/cable/vehicle/virtual platforms, expressive candidate selection, anti-stereotype memory, and platform handoffs: [cinematography-language-engine.md](references/cinematography-language-engine.md)
...
- Run `cinematography-language-engine.md` before detailed coverage. Select viewpoint, visual distance, position/roll, support, optics, and movement as separate decisions.
- For scenes with meaningful camera-expression space, compare an objective/safe, performance-led, and expressive candidate internally. First-person, semi-subjective, Dutch, zoom, Steadicam, handheld, body-rig, drone, FPV, cable/vehicle, or virtual-camera options must enter consideration when their activation conditions fit; do not wait for the user to name them.
...
3. A concise Camera Language Brief when cinematography is material: viewpoint arc, camera temperament, visual-distance curve, support/optical palette, expressive technique or hero event, handoffs, and anti-defaults.
4. Shot table with timecode, viewpoint, framing, world-space camera position, support, optics or graphic perspective, trigger/path/landing, blocking/pose, image, sound, and continuity notes.
...
7. For hyperreal combat, include performer-camera trigger/lead-lag logic and the sequence's key Hero Camera Move when relevant.
