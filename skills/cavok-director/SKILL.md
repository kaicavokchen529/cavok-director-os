...
- Production readiness, evidence-state separation, spatial previs, light/depth gates, geometry-before-lens, evidence boundaries, director mechanism anti-triggers, and matched-dimension calibration: [production-previs-evidence-gates.md](references/production-previs-evidence-gates.md)

### Camera, optics and shot design

- Scene-level cinematography strategy, first-person/semi-subjective/objective/aerial regimes, shot-size arc, high/low/overhead angles, Dutch angle, axial/lateral/rotational/vertical moves, zoom family, bullet time/speed ramps, Steadicam/handheld/body rigs, drone/FPV/cable/vehicle/virtual platforms, expressive candidate selection, anti-stereotype memory, and platform handoffs: [cinematography-language-engine.md](references/cinematography-language-engine.md)
...
- Run `cinematography-language-engine.md` before detailed coverage. Select viewpoint, visual distance, position/angle/roll, support, optics, movement topology, and temporal behavior as seven separate decisions.
- For scenes with meaningful camera-expression space, compare an objective/safe, performance-led, and expressive candidate internally. First-person, semi-subjective, high/low/overhead angles, Dutch, physical push/pull, crash zoom, Steadicam, handheld, body-rig, drone, FPV, cable/vehicle, bullet time, speed ramp, or virtual-camera options must enter consideration when their activation conditions fit; do not wait for the user to name them.
- Distinguish physical camera travel from optical change, angle state from movement, and bullet time from ordinary slow motion. “急推”“后拉”“俯拍” or similar shorthand must be resolved into observable mechanics before delivery.
- Design a viewpoint and platform arc across the scene. Any objective↔subjective or aerial↔ground handoff must preserve or deliberately transform target, direction, speed, space, and audience knowledge.
...
- Time manipulation is optional, not automatic. Bullet time, slow motion, speed ramps, undercrank, freeze, or subjective time dilation must define subject/world/camera/audio time, an exact entry trigger, what becomes newly readable, and the return-to-time state. Respect any project-level no-slow-motion rule.
- Hyperreal action may use bold camera showmanship—weapon pass, body pass, ground skim, camera crash-in, axial subject rush with yielding retreat, vertical rise/dive, hidden cut, motivated axis crossing, reverse pull, vector match, shot-inside-shot—when each technique is visibly caused by character movement, force, environment, or information change.
- Use realistic support in live-action mode: Tripod, Dolly, Slider, Steadicam, Handheld, Gimbal, Crane/Jib, Drone, FPV Drone, Cable Cam, Vehicle Rig, Body Rig, Snorricam, or motivated head/chest/weapon rigs. Stylized CG virtual cameras may exceed physical rigs, but still require motivation, inertia and spatial logic.
...
- Reserve camera stunts for beats that benefit from them. Establish a clear hierarchy between functional coverage, designed movement, and any Hero Camera Move; do not impose a universal stunt count or keep the sequence at maximum flair.
...
- **Evidence State:** keep Fact, direct Observation, Inference, Director Proposal, and Unknown distinct; never silently promote inference into project truth.
- **Adaptive Production Readiness:** simple scenes may stay lightweight, but complex geography, repeated reverse coverage, ensemble action, or reference reconstruction must resolve upstream dependencies before detailed camera design.
- **Spatial Previs:** when geography materially affects action or camera, establish one world-space base before screen-space composition; reverse views belong to the same world and are not mirrored rooms.
- **Light Source Causality:** important photographic light must identify source, scene anchor, direction, landing, and falloff / occlusion.
- **Dominant Depth Mechanism:** depth-heavy shots choose one primary cause—light/exposure separation, atmospheric perspective, or occlusion/scale recession—rather than stacking generic cinematic depth effects.
- **Volumetric Double Gate:** visible beams, fog-lit shafts, or glowing suspended media require both a real medium and a named directional source.
- **Director Mechanism + Anti-Trigger:** store reusable directing knowledge as activation conditions, dramatic problem, causal mechanism, visible result, exit condition, and surface traits that are insufficient to trigger it.
- **Geometry Before Lens:** solve crop, subject occupancy, camera/subject/background relationship, perspective and occlusion before using focal length as the corrective lever.
- **Evidence Boundary:** a still frame cannot prove camera motion, duration, cut timing or full performance arc; visual validation is claimed only after actual output has been inspected.
- **Matched Dimension Lock:** on retries, identify what already works, isolate the largest current failure, change one cause group, and protect matched dimensions from regression.

Full rules: [production-previs-evidence-gates.md](references/production-previs-evidence-gates.md).
...
3. A concise Camera Language Brief when cinematography is material: viewpoint arc, camera temperament, visual-distance curve, support/optical/temporal palette, expressive technique or hero event, handoffs, and anti-defaults.
