...
| Scene camera language, viewpoint arc, visual-distance curve, angle/elevation state, support/optical/temporal palette, expressive candidates and platform handoffs | `cinematography-language-engine.md` | shot, movement, optics, medium and model modules |
| Shot framing and world-space camera position | `camera-shot-decision-system.md` | optics, movement grammar |
| Camera movement topology execution, curves, support behavior and approved temporal-rate execution | `camera-movement-grammar.md` | hyperreal action, shot plans |
